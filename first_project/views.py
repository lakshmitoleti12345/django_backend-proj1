...
def jsonpage(request):
    data = {
        'pic': "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSuUZef4D3XGsJLlCvLlBrgSIEK0ZFtq7TIfcB-ZMe5PQ&s=10",
        'name': "Yesh",
        'post': 140,
        'followers': 9338,
        'following': 10,
        'bio': 'Dream big. Work hard. Repeat. 🚀 Building the life I imagined. Future under construction ⚡',
        'background': "#DDE5B6"
    }
...
def demopage(request):
    ...
    return render(request, 'laxmi.html')
def login(request):
    print(request.POST)
    if(request.method=='GET'):
...
def get_students(request):
    return render(request,'students.html',{'names':['satya','sai','satish','shakthi']})
